...
from configuration import DATA_SOURCE_PATH, LANGUAGES_SHORT, LANGUAGES, PROMPTS, ROOT_PATH
...
def get_model_predictions(model: str, prompt_name: str = "Prompt 3"):
    predictions_path = Path(DATA_SOURCE_PATH, "predictions_titles", METHOD_NAME, model)
    ...
    model_translation_times_path = Path(ROOT_PATH, "results", "title_translation_times.csv")
    ...
    for label_data_path in sorted(Path(DATA_SOURCE_PATH, "labels_titles").iterdir()):
...
def benchmark_model_translations(model: str, prompt_name: str = "Prompt 3"):
    ...
    benchmark_result_path = Path(ROOT_PATH, "results", "title_translation_benchmarks.csv")
