...
from configuration import DATA_SOURCE_PATH, PREDICTIONS_SOURCE_PATH, LANGUAGES_SHORT, LANGUAGES, PROMPTS, ROOT_PATH
...
def get_predictions_for_file(data: dict, model: str, prompt_name: str = "Prompt 3"):
    page_numbers = set([x["page_number"] for x in data["paragraphs"]])
    prediction_data = {
    ...
    for page_number in sorted(page_numbers):
        page_paragraphs = [x for x in data["paragraphs"] if x["page_number"] == page_number]
        main_language_text = "\n\n".join([x["main_language"] for x in page_paragraphs])
        other_language_text = "\n\n".join([x["other_language"] for x in page_paragraphs])
        page_prediction = get_prediction(
            model, main_language_text, data["main_language"], data["other_language"], prompt_name
        )
        prediction_data["paragraphs"].append(
            {
                "main_language": main_language_text,
                "other_language": other_language_text,
...
def get_model_predictions(model: str, prompt_name: str = "Prompt 3"):
    ...
    for label_data_path in sorted(Path(DATA_SOURCE_PATH, "labels_with_segment_info").iterdir()):
