...
METHOD_NAME = "segment_to_segment_with_titles"
...
def get_content(translation_task: TranslationTask, document_title: str, prompt_name: str = "Prompt 3"):
    ...
    content = PROMPTS[prompt_name].format(
        language_to_name=language_to_name, text_to_translate=translation_task.text, document_title=document_title
    )
    return content


def get_prediction(
    model: str, text: str, language_from: str, language_to: str, document_title: str, prompt_name: str = "Prompt 3"
):
    translation_task = TranslationTask(text=text, language_from=language_from, language_to=language_to)
    content = get_content(translation_task, document_title, prompt_name)
    response = Client().chat(model=model, messages=[{"role": "user", "content": content}])
    return response["message"]["content"]


def get_predictions_for_file(data: dict, model: str, document_title: str, prompt_name: str = "Prompt 3"):
    for paragraph in data["paragraphs"]:
        prediction = get_prediction(
            model, paragraph["main_language"], data["main_language"], data["other_language"], document_title, prompt_name
        )
...
def get_model_predictions(model: str, prompt_name: str = "Prompt 3"):
    ...
    for label_data_path in sorted(LABELS_SOURCE_PATH.iterdir()):
        ...
        title_data_path = Path(DATA_SOURCE_PATH, "labels_titles", label_data_path.name)
        ...
        title_data = load_data(title_data_path)
        document_title = title_data["paragraphs"][0]["main_language"]
        start_time = time()
        data = get_predictions_for_file(data, model, document_title)
