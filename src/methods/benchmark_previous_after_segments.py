...
METHOD_NAME = "previous_after_segments"
...
def get_content(
    translation_task: TranslationTask, prompt_name: str = "Prompt 3", previous_text: str = "", next_text: str = ""
):
...
def get_prediction(
    ...
    language_from: str,
    language_to: str,
    prompt_name: str = "Prompt 3",
):
    translation_task = TranslationTask(text=text, language_from=language_from, language_to=language_to)
    content = get_content(translation_task, prompt_name, previous_text, next_text)
...
def get_predictions_for_file(data: dict, model: str, prompt_name: str = "Prompt 3"):
    ...
    prev_paragraphs = [None] + data["paragraphs"][:-1]
    next_paragraphs = data["paragraphs"][1:] + [None]

    for prev_paragraph, paragraph, next_paragraph in zip(prev_paragraphs, data["paragraphs"], next_paragraphs):
        previous_text = "[empty]"
        if prev_paragraph:
            previous_text = prev_paragraph["main_language"]

        next_text = "[empty]"
        if next_paragraph:
            next_text = next_paragraph["main_language"]
...
def get_model_predictions(model: str, prompt_name: str = "Prompt 3"):
    ...
    for label_data_path in sorted(LABELS_SOURCE_PATH.iterdir()):
        ...
        start_time = time()
        data = get_predictions_for_file(data, model, prompt_name)
...
if __name__ == "__main__":
    # get_model_predictions("gpt-oss", "Prompt 4")
    benchmark_model_translations("gpt-oss", "Prompt 4")
